...
namespace TPM3.zxd
{
    public partial class TestTypeForm : MyBaseForm
    {
        ...
        private void grid2_OnAddNew(object sender, EventArgs e)
        {
            ...
            grid2.Columns["充分性要求"].Value = DefaultText.GetItemSufficientDefaultText();
            this.grid2.Columns["终止条件"].Value = DefaultText.GetItemTerminateDefaultText();
            grid2.Columns["评判标准"].Value = DefaultText.GetItemEvaluateDefaultText();
            grid2.Columns["约束条件"].Value = DefaultText.GetItemConstrainDefaultText();
